...
One caveat: make the calls with an OpenAI-compatible client (the `openai` SDK, or the Vercel AI SDK's OpenAI-compatible provider pointed at the gateway URL). The AI SDK's native gateway wiring (plain `"provider/model"` strings) uses a protocol the Belvedir SDK doesn't instrument, so those calls would arrive as generic spans without prompts or token counts.
...
2. In the Belvedir dashboard, open your project's settings, go to **Integrations**, click the **Vercel AI Gateway** preset on the AI gateway card, and paste the key.
