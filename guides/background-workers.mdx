...
In Python, `flush()` is synchronous and returns once the export has completed, so calling it immediately before `os._exit()` (or a hard exit in a signal handler) is safe. In Node, `flush()` is `async`: `await flush()` before the process exits, or the promise is abandoned along with the batch. The hazard is exiting *without* it: `os._exit`, `process.exit()`, and `SIGKILL` skip exit hooks and kill the exporter, so an unflushed batch is silently lost.
