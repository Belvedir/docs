...
MODEL_API_BASE  # OpenAI-compatible base URL; leave blank to infer the
                # provider from the model id (claude-* → Anthropic,
                # gpt-* → OpenAI)
MODEL_API_KEY   # key for that endpoint, billed to your account
```

Any OpenAI-compatible endpoint works: hosted providers, a Together-deployed fine-tune, or your own vLLM. LLM-judged drivers take the judge separately as `JUDGE_MODEL` / `JUDGE_API_BASE` / `JUDGE_API_KEY`, so the grader stays fixed while the model under test varies. GPU drivers (lm-eval, MT-Bench) instead serve `MODEL` in-sandbox with vLLM and need no model endpoint.
