...
The New Harness page offers one-click presets for common suites — SWE-bench Lite, τ-bench, Terminal-Bench, GAIA, Aider Polyglot, lm-eval and MT-Bench (vLLM-served) — and for agent scaffolds: OpenClaw, Hermes Agent, OpenCode, Prime Agent, Pi, and QM. Every prefilled field is editable before you create the harness. Scaffold drivers bring the agent but no task suite: you author the tasks in the harness's Tasks field, and the sandbox exposes them to your run command as a file at `$BELVEDIR_TASKS_FILE`.
