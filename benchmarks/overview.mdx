...
Complete suites like SWE-bench bring their own tasks and grading. The Harbor-format suites (SWE-bench Verified, Terminal-Bench 2.1, τ³-bench, GAIA, Aider Polyglot) run through [Harbor](https://harborframework.com): pick the agent scaffold with `HARBOR_AGENT` (terminus-2 by default, or any Harbor agent such as claude-code), cap the run with `HARBOR_N_TASKS` (5 by default; Terminal-Bench tasks take several minutes each and a run is capped at one hour), and, unless the deployment provides a managed Harbor workspace, supply `MODAL_TOKEN_ID` / `MODAL_TOKEN_SECRET`: each task runs in its own container on Modal, and Belvedir never places its own platform credentials inside a sandbox. Scaffold-style harnesses like OpenClaw are **custom benchmarks**: you supply the task list (the Tasks field on the harness) and an LLM judge grades the outcomes, so you can benchmark exactly the work your agent does in production.

## Start from the catalog

The New Harness page offers one-click presets, starting with the first-party **Belvedir Runner** (a minimal agent loop with a pinned independent LLM judge) and **Belvedir Runner on Harbor** (the same loop and judge, one container per task; the default driver for [generated environments](/benchmarks/environments) on deployments with a managed Harbor workspace, with the in-sandbox runner the default elsewhere), alongside the Harbor-format suites (SWE-bench Verified, τ³-bench, Terminal-Bench 2.1, GAIA, Aider Polyglot) and agent scaffolds run through Harbor: OpenClaw, OpenCode, and Pi today. Some cards are listed but marked **Not yet available** because their drivers are not published yet: the GPU-served lm-eval and MT-Bench presets, and the Hermes Agent, Prime Agent, QM, and Mastra scaffolds. Those can't be added from the page or with `belvedir benchmarks add`, and can't back an environment. Every prefilled field is editable before you create the harness. The Belvedir Runner and the scaffold drivers bring the agent but no task suite: you author the tasks in the harness's Tasks field, and the sandbox exposes them to your run command as a file at `$BELVEDIR_TASKS_FILE`.
