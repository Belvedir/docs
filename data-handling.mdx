...
- **Traces are stored as your instrumentation sends them.** Every exported span is kept, including prompt and completion text when your SDK or OTel setup emits it (the default). Spans are linked into sessions, and sessions are segmented into tasks, which keep a plain-text input/output distillation plus an embedding. The Belvedir SDKs have no redaction switch: what your instrumentation emits is what is stored, so strip or omit sensitive content in your app before it reaches the exporter. The [OpenClaw and Hermes plugins](/guides/otel-agents) do have one: set `BELVEDIR_PRIVACY_MODE=true` in the agent's environment and their spans carry usage and structure only. Ingest error logs never include span content, only span counts.
- **Trace data has no automatic expiry.** Traces, sessions, and tasks are retained until you delete them; deleting a project removes its traces, sessions, tasks, and task groups with it. The metering rows behind the organization's usage and billing history (model, token counts, cost) are kept, with their project reference cleared.
- **Batch inference results are retained for 29 days** after a batch ends, then purged (the batch's counts and spend remain). To remove results sooner, delete the ended batch with `DELETE /api/v1/messages/batches/{id}` (the Anthropic-facade endpoint; it accepts any ended batch of the project, whichever endpoint created it). `/api/v1/route/batches` has no delete of its own. See [batch inference](/api-reference/route#post-apiv1routebatches-batch-inference-at-half-price).
...
- **Three more per-project gates apply to every run**, automatic or manual: the **Model training** permission turns training off entirely; the **Data sources** selection in the Training setup wizard's first step (**Receive traces**) restricts which API keys' traffic is eligible (leaving every key selected means all of the project's own traffic); and sessions you report as `"fail"` via [outcomes](/api-reference/outcomes) are excluded from training data.
...
| Token compression | Project Permissions | On by default for new projects: prompts on routed calls transit the compressor while it is on; turn it off to keep prompt text out of it |
| Model training | Project Permissions | Off: no training runs, manual or automatic |
| Harness access | Project Permissions | Off: training loops cannot read your agent's repository or propose changes as pull requests |
| Data sources | Training setup wizard, **Receive traces** step | Limits a loop's training data to traffic from the selected API keys |
| AI gateway | Project settings → Integrations | All routed execution moves to your own upstream and account |
| Privacy mode (OpenClaw and Hermes plugins only) | `BELVEDIR_PRIVACY_MODE` env var in the agent's environment | Plugin spans export usage and structure only, no content. The SDKs have no equivalent switch |
