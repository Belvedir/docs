...
- **Trace data has no automatic expiry.** Traces, sessions, and tasks are retained until you delete them; deleting a project removes its traces, sessions, tasks, and training sets with it. The metering rows behind the organization's usage and billing history (model, token counts, cost) are kept, with their project reference cleared.
