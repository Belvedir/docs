...
Migrating from Fractal? The old npm packages (`@fractalresearch/loop`, `@belvedir/loop`) still install but are deprecated; on PyPI, `belvedir-loop` is frozen at 0.4.0. Use `belvedir`.
...
This is what powers tasks and task groups. Wrap each agent run in `withSession` so every LLM and tool call is linked under one session id. Belvedir then segments that session into the individual tasks the agent performed. Optionally wrap distinct units of work in `task()` for sharper task boundaries:
