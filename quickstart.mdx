...
Until your project has a [router](/api-reference/route) configured, the endpoint is a plain metered proxy that honors the `model` you pass, so this swap is safe to ship first. Once routing is configured, Belvedir picks the model per request (your fine-tuned models included), runs the call, and bills your organization at cost.
