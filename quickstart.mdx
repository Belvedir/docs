...
Then create an API key with Quick Start's Generate key button or from the dashboard's API Keys page; keys start with `bv_live_`. Save it somewhere secure; it won't be shown again. (Older keys starting with `fr_live_` remain valid.)
