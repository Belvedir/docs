...
Every project starts with a [router](/api-reference/route), so Belvedir picks the model per request (your fine-tuned models included), runs the call, and bills your organization at cost. Pass `model: "auto"` and let it decide: naming a specific provider model here does nothing to the routing, but it does label every trace with a model that never ran. Only a project whose routers were all deleted honors an explicit `model`, as a plain metered proxy.
