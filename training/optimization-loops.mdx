...
```
  ...
  your agent in production                              │
             │  every LLM + tool call (the belvedir SDK)│
             ▼                                          │
  traces ──► sessions ──► tasks ──► training sets       │
                            │                           │
...
- **LoRA finetuning**: train lightweight LoRA adapters on your traces instead of touching the full weights: cheap to run, quick to iterate, and swappable per task type, on top of the supported open base models. See [Model Training](/training/overview).
...
```
  ...
  for each failure mode:
    ...
    edits (and brand-new files) for that mode
       │
       ▼
  SCORE each patch on real traces: where the
  patch touches what the model was told, the
  recorded model call is RE-RUN with the patch
  applied and both outputs are compared blind,
  in both orders. Keep only patches that improve
  the failing traces without regressing the OK
  ones, then merge the compatible winners
       │
       ▼
