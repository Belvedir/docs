...
If the endpoint knows the model under a different id (a Together deploy's generated name, or a provider model you want to serve under a name of your own), fill in **Model id your endpoint expects**: requests go out with that id, while routers, the Playground and the usage graphs keep the name you registered. Leave it blank when the ids match.
