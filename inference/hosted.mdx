...
If the endpoint knows the model under a different id (a Together deploy's generated name, or a provider model you want to serve under a name of your own), fill in **Model id it runs as**: requests go out with that id, while routers, the Playground and the usage graphs keep the name you registered. Leave it blank when the ids match.

Leave the base URL blank instead, and the same field names a hosted model Belvedir serves: your name then runs on Belvedir's account as that model, billed at its rate like any hosted call, with no key of yours stored.

## Default gateway (optional)

Project settings → **Integrations** can register one OpenAI-compatible gateway for every routed model at once: every call the router executes then runs against your gateway with your key, instead of Belvedir's provider accounts. Use it when you already aggregate AI spend somewhere, such as [Vercel AI Gateway](https://vercel.com/docs/ai-gateway) or an OpenRouter account: your gateway's dashboard sees all traffic and spend, while Belvedir keeps routing, tracing, and metering the calls. See the [Vercel AI Gateway guide](/guides/vercel-ai-gateway) for the end-to-end setup.
