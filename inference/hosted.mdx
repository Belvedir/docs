...
Models trained on Belvedir are standard PEFT LoRA adapters. Click **Deploy** on the model's row (Models page) and paste a Together API key: the adapter is uploaded and served automatically from a scale-to-zero endpoint (the key is used for the deploy and never stored; the endpoint lands in your Together account). Then point your agent at that endpoint exactly as above. Prefer somewhere else? The panel also has prefilled steps for Fireworks and self-hosted vLLM, and the adapter downloads as a plain PEFT zip that loads anywhere. Belvedir is never in the request path. One caveat: adapters trained on the Qwen3.5-9B base target hybrid-attention modules that Together's serving stack doesn't support, so its validation refuses them; serve those from the inference playground, which merges the adapter into the base weights automatically.