...
| Routing (`"auto"`, tiers, training sets) | Yes, while the project's **Smart routing** permission is on (the default). Off = every call serves exactly the model it selects, as tier `fixed` | No: model-pinned by design | No: every request names its model |
