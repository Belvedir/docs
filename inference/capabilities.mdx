...
| Token compression (prompt compression) | Yes: the per-project **Token compression** toggle, on by default for new projects | No: the body is never touched | No |
...
- **Embeddings**: any OpenAI `text-embedding-*` id is accepted, but an unpriced one returns `400`; the priced ones today are `text-embedding-3-small`, `text-embedding-3-large`, and `text-embedding-ada-002`.
...
- **`tool-choice-auto`**: the serving model rejects forced tool choice (Claude Fable 5.1 and Claude Opus 5.5 return `400` on `tool_choice: "required"` or a named function), so Belvedir sent the call with `tool_choice: "auto"` instead. The tools stay declared; steer the model toward the tool in your prompt, and do not assume a `tool_calls` finish. No first attempt is metered: the downgrade happens before the call. Batches apply the same downgrade but their results carry no marker; only the sync router sets this header.
...
| Request body: batch submission | ~4 MB inline (the platform edge limit, `413` beyond it); larger submissions go by reference through `POST /api/v1/route/batches/uploads` |
...
| Rate: batch submissions | 30 per minute per API key, burst 30 (status polls: 60 per minute, burst 120; results reads are not rate-limited) |
