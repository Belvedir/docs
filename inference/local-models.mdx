...
Download the **Belvedir Local** desktop app; it bundles the Ollama engine, can also drive [llama.cpp](https://github.com/ggml-org/llama.cpp) or [vLLM](https://docs.vllm.ai) for models you point it at, and walks you through picking a model that fits your machine's memory.

<Note>
macOS warns on first open (the app is ad-hoc signed): right-click the app, choose Open, then Open again. Or clear the flag with `xattr -cr "/Applications/Belvedir Local.app"`.
</Note> If you already run [Ollama](https://ollama.com) yourself, that works too (e.g. `ollama pull qwen3:4b`). Either way a local server listens on `127.0.0.1:11434`, Ollama's default. If you've set `OLLAMA_HOST` or run Ollama in Docker with a remapped port, use that address in the `baseURL` below instead.
