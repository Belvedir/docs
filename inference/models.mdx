...
Every model Anthropic's API serves works here, not just the table: the older Claude Opus 4.5 (`anthropic/claude-opus-4-5`) and Claude Sonnet 4.5 (`anthropic/claude-sonnet-4-5`) are served and priced too, and dated snapshot ids resolve to the same rates.
...
Beyond the table, every chat model OpenAI's API serves works here and is priced at OpenAI's list: the GPT-5.5 and GPT-5.4 families, GPT-5.1, the pro models (`gpt-5.2-pro`, `gpt-5-pro`, ...), the codex and chat-latest variants, the o-series (`o1`, `o3`, `o3-mini`, `o4-mini`), and the legacy line back through `gpt-4-turbo`, `gpt-4`, and `gpt-3.5-turbo`.
...
| Qwen 3.6 35B | `Qwen/Qwen3.6-35B-A3B` | Sail Research (American compute) |
| Nemotron 3 Super 120B | `nvidia/NVIDIA-Nemotron-3-Super-120B-A12B-BF16` | Sail Research (American compute) |
