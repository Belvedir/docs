...
The Playground is the dashboard's try-it surface. Open **Playground** in the sidebar (in the top group, after **API Keys** and **Models**), pick a model, and send a message. Add up to four models and every message you send goes to all of them at once, so you can compare answers, speed, and cost on the same prompt.
...
Fine-tuned models don't run in the Playground until you deploy them on your own provider and register the endpoint on the Cloud Inference page. Local models never run here. A registration can name the id the endpoint expects when it differs from the model's name, so a fine-tuned model shows up here under its own name whatever the deploy called it.
